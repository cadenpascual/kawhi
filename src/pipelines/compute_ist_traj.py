...
def process_single_play(row, params):
    """
    Runs the JKO simulation for a single play, extracts the simulated defender
    trajectories, un-flips the coordinates back to SportVU standard, and calculates IST metrics.
    """
    try:
        ...
        for i in range(5):
            new_data[f'sim_def{i+1}_x_traj'] = (sim_def_np[:, i, 1] - 25.0).tolist()
            new_data[f'sim_def{i+1}_y_traj'] = (sim_def_np[:, i, 0] - 5.25).tolist()
        ...
        if weights_real.ndim == 2:
            weights_real = np.sum(weights_real, axis=1)
            weights_sim_ist = np.sum(weights_sim_ist, axis=1)

        # 1. Total IST Math
        total_real = float(np.sum(np.where(mask, weights_real, 0.0)))
        total_sim = float(np.sum(np.where(mask, weights_sim_ist, 0.0)))
        
        # 2. Frame Win Rate Math (Sim wins if it keeps the Threat/IST lower than Real)
